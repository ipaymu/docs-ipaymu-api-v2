import { getPageImage, pluginsSource } from "@/lib/source";
...
export default async function Page(props: { params: Promise<{ slug?: string[]; lang: string }> }) {
  ...
  const page = pluginsSource.getPage(params.slug, params.lang);
    ...
    >
      ...
      <DocsBody>
        <MDX
          components={{
            ...getMDXComponents({
              a: createRelativeLink(pluginsSource, page),
            }),
...
export async function generateStaticParams() {
  const params = await pluginsSource.generateParams();
  return params.map((p) => ({
    ...p,
