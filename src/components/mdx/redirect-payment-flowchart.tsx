...
const REDIRECT_STEPS: StepItem[] = [
  {
    id: 1,
    title: { id: 'Request Redirect Payment', en: 'Request Redirect Payment' },
    desc: {
      id: 'Merchant mengunggah detail pesanan ke /api/v2/payment disertai returnUrl, notifyUrl & cancelUrl.',
      en: 'Merchant uploads order details to /api/v2/payment along with returnUrl, notifyUrl & cancelUrl.',
    },
    badge: 'POST /payment',
  ...
  {
    id: 2,
    title: { id: 'Dapatkan SessionID & Url Checkout', en: 'Receive SessionID & Checkout Url' },
    desc: {
      id: 'iPaymu mengembalikan SessionID & Url pembayaran hosted checkout resmi iPaymu.',
      en: 'iPaymu returns SessionID & hosted checkout payment Url.',
    },
    badge: 'Checkout Url',
  ...
  {
    id: 3,
    title: { id: 'Redirect Pelanggan ke iPaymu', en: 'Redirect Customer to iPaymu' },
    desc: {
      id: 'Sistem Merchant mengarahkan (redirect) pelanggan ke Halaman Pembayaran Hosted iPaymu.',
      en: 'Merchant system redirects customer to iPaymu Hosted Payment Page.',
    },
    badge: 'Browser Redirect',
    icon: <MousePointerClick className="w-5 h-5 text-amber-600 dark:text-amber-400" />,
  ...
  {
    id: 4,
    title: { id: 'Pilih Kanal & Bayar', en: 'Select Channel & Pay' },
    desc: {
      id: 'Pelanggan memilih kanal pembayaran favorit (VA / e-Wallet / QRIS / CC) di UI iPaymu dan menyelesaikan transaksi.',
      en: 'Customer selects preferred payment channel (VA / e-Wallet / QRIS / CC) on iPaymu UI & completes transaction.',
    },
    badge: 'Hosted Checkout UI',
  ...
  },
  {
    ...
    desc: {
      id: 'iPaymu mengirim HTTP POST notifikasi callback ke notifyUrl milik Merchant.',
      en: 'iPaymu sends HTTP POST callback notification to Merchant notifyUrl.',
    },
    badge: 'HTTP Callback',
  ...
  {
    id: 6,
    title: { id: 'Redirect Kembalikan Pelanggan', en: 'Redirect Back to Merchant' },
    desc: {
      id: 'iPaymu mengarahkan kembali pelanggan ke returnUrl (Halaman Berhasil) milik Merchant.',
      en: 'iPaymu redirects customer back to Merchant returnUrl (Success Page).',
    },
    badge: 'returnUrl Complete',
...
export function RedirectPaymentFlowchart({ lang = 'id' }: { lang?: 'id' | 'en' }) {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-base leading-none tracking-tight">
              {isEn ? 'Redirect Payment Integration Flow' : 'Alur Integrasi Pembayaran Redirect'}
            </h3>
            <p className="text-xs text-muted-foreground mt-1">
              {isEn
                ? '6 sequential steps using iPaymu hosted checkout page'
                : '6 tahapan integrasi pembayaran halaman ter-host iPaymu'}
            </p>
          </div>
        </div>
